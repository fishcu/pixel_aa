...
void rasterize_center(uint32_t *in, uint32_t *out, int in_pitch, int out_pitch,
  ...
  for (int y = begin_y; y < end_y; ++y) {
    ...
    if (offset_y < OFFSET_TOL) {
      ...
      col = GET_COL(mix(GET_CH(in[in_row_offset], 0),
    ...
    for (int x = border_x; x < out_width - border_x; ++x, in_x += in_x_step) {
      ...
      if (offset_y < OFFSET_TOL) {
        if (offset_x < OFFSET_TOL) {
          ...
          out[out_row_offset + x] = GET_COL(
              mix(GET_CH(*in_ptr[0], 0), GET_CH(*in_ptr[1], 0), offset_x),
              mix(GET_CH(*in_ptr[0], 1), GET_CH(*in_ptr[1], 1), offset_x),
              mix(GET_CH(*in_ptr[0], 2), GET_CH(*in_ptr[1], 2), offset_x));
        }
      } else if (offset_y > 1.0f - OFFSET_TOL) {
        if (offset_x < OFFSET_TOL) {
          // Need 1 sample, no mixing
          ...
          // Need 2 samples, mix with offset_x
          out[out_row_offset + x] = GET_COL(
              mix(GET_CH(*in_ptr[2], 0), GET_CH(*in_ptr[3], 0), offset_x),
              mix(GET_CH(*in_ptr[2], 1), GET_CH(*in_ptr[3], 1), offset_x),
              mix(GET_CH(*in_ptr[2], 2), GET_CH(*in_ptr[3], 2), offset_x));
        }
      } else {
        if (offset_x < OFFSET_TOL) {
          // Need 2 samples, mix with offset_y
          out[out_row_offset + x] = GET_COL(
              mix(GET_CH(*in_ptr[0], 0), GET_CH(*in_ptr[2], 0), offset_y),
              mix(GET_CH(*in_ptr[0], 1), GET_CH(*in_ptr[2], 1), offset_y),
              mix(GET_CH(*in_ptr[0], 2), GET_CH(*in_ptr[2], 2), offset_y));
        } else if (offset_x > 1.0f - OFFSET_TOL) {
          // Need 2 samples, mix with offset_y
          out[out_row_offset + x] = GET_COL(
              mix(GET_CH(*in_ptr[1], 0), GET_CH(*in_ptr[3], 0), offset_y),
              mix(GET_CH(*in_ptr[1], 1), GET_CH(*in_ptr[3], 1), offset_y),
              mix(GET_CH(*in_ptr[1], 2), GET_CH(*in_ptr[3], 2), offset_y));
        } else {
          // Need 4 samples, mix with offset_x and offset_y
          out[out_row_offset + x] = GET_COL(
              mix(mix(GET_CH(*in_ptr[0], 0), GET_CH(*in_ptr[1], 0), offset_x),
                  mix(GET_CH(*in_ptr[2], 0), GET_CH(*in_ptr[3], 0), offset_x),
                  offset_y),
              mix(mix(GET_CH(*in_ptr[0], 1), GET_CH(*in_ptr[1], 1), offset_x),
                  mix(GET_CH(*in_ptr[2], 1), GET_CH(*in_ptr[3], 1), offset_x),
                  offset_y),
              mix(mix(GET_CH(*in_ptr[0], 2), GET_CH(*in_ptr[1], 2), offset_x),
                  mix(GET_CH(*in_ptr[2], 2), GET_CH(*in_ptr[3], 2), offset_x),
                  offset_y));
        }
      }
    ...
    if (offset_y < OFFSET_TOL) {
      ...
      col = GET_COL(
          mix(GET_CH(in[in_row_offset + in_width - 1], 0),
              GET_CH(in[in_row_offset + in_pitch + in_width - 1], 0), offset_y),
          mix(GET_CH(in[in_row_offset + in_width - 1], 1),
              GET_CH(in[in_row_offset + in_pitch + in_width - 1], 1), offset_y),
...
int main(int argc, char *argv[]) {
  ...
  std::unique_ptr<unsigned char[]> out_img_data(new unsigned char[output_size]);
  ...
  */
  ...
  const int border_y = out_height >= in_height ? out_height / in_height - 1 : 0;
  ...
  for (int perf_pass = 0; perf_pass < num_perf_passes; ++perf_pass) {
    ...
    */
    ...
    for (int i = 0; i < num_threads; ++i) {
      int begin_y = border_y + (out_height - 2 * border_y) * i / num_threads;
      int end_y = border_y + (out_height - 2 * border_y) * (i + 1) / num_threads;
      // printf("pass %d\nlaunching thread %d, from %d to %d\n", perf_pass, i,
      //        begin_y, end_y);
      threads.emplace_back(rasterize_center, in_img_data, out, in_width, out_width,
                      in_width, out_width, border_x, begin_y, end_y,
                      weights_x.get(), weights_y.get(), in_x_step, in_y_step);
    }
    ...
    for (int y = out_height - border_y; y < out_height; ++y) {
      ...
      float in_x = (border_x + 0.5f) * in_x_step - 0.5f;
      int in_sample_x = int(in_x);
      uint32_t *in_ptr[2] = {in_img_data + in_row_offset + in_sample_x,
      ...
      for (int x = out_width - border_x; x < out_width; ++x) {
        out[out_row_offset + x] = in_img_data[in_row_offset + in_width - 1];
      }
    }
  ...
  std::string directory = input_path.substr(0, input_path.find_last_of("/\\"));
  ...
  std::string output_path = directory + "/" + output_file_name + "_output.png";
